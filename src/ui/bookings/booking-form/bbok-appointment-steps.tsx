...
const BookAppointmentSteps = ({
  ...
  switch (currentStep) {
    case 1:
      return (
        <BookAppointmentEmail register={register} errors={errors} />
      )
    case 2:
      return (
        <BookAppointmentDate form={form} />
      )
    case 3:
      return (
        <BookAppointmentDay
...
function BookAppointmentEmail ({
  ...
  return (
    <div>
      ...
      <Button
        type="submit"
        className="border border-border hover:bg-surface text-text mt-4  w-full lg:max-w-[400px]"
        onClick={handleNext}
      >
        Next
      </Button>
...
function BookAppointmentDay ({
  ...
  useEffect(() => {
    async function getAvailableTimes() {
      try {
        ...
        const existingBookings = await getBookingsByDate(domainId, selectedDate)
  ...
  return (
    <div className="flex flex-col items-center">
      <span className="text-text text-sm font-medium">Pick a Time</span>
      <DaypickerInput
        times={times}
      ...
      <div className="w-full flex gap-4 justify-between mt-8">
        <Button
          className="hover:bg-surface text-text mt-4"
          onClick={() => {
            setCurrentStep(prev => prev - 1)
          }}
        >
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem className="flex flex-col items-center">
          <FormLabel className="text-text text-sm font-medium">Pick a Date</FormLabel>
            <Calendar
              mode="single"
...
function DaypickerInput({
  ...
  return (
    <ul className={cn("w-full grid grid-cols-3 gap-4 ", className)}>
      {
        times.map(d => (
          <Button
            key={d}
            className={cn("border border-border rounded-md text-text p-4 hover:bg-surface disabled:border-0", {'bg-surface': time === d})}
            type="button"
            onClick={() => setTime(d)}
